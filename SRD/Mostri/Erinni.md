---
id: "srd-erinni"
srd_id: "erinni"
nome: "Erinni"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Erinni"
type: "Immondo (diavolo)"
size: "medio"
alignment: "legale malvagio"
ac: 18
stats: [18, 16, 18, 14, 14, 18]
saves: 
  dex: 7
  con: 8
  cha: 8
skillsaves: 
  percezione: 6
  persuasione: 8
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 16, vista_pura: 36 m"
languages: "Infernale, telepatia 36 m"
cr: 12
traits: 
- name: "Corda magica"
  desc: "L'erinni possiede una corda magica. Finché la porta con sé, può usare l'azione Corda avvinghiante. La corda ha CA 20, 90 PF, e immunità ai danni psichici e da veleno. La corda si trasforma in polvere se viene ridotta a 0 punti ferita, se si trova a 1,5 metri o più dall'erinni per 1 ora o più, o se l'erinni muore. Se la corda è danneggiata o distrutta, l'erinni può ripristinarla interamente al termine di un riposo breve o lungo."
- name: "Resistenza alla magia"
  desc: "L'erinni dispone di vantaggio ai tiri salvezza contro incantesimi e altri effetti magici."
- name: "Ristoro diabolico"
  desc: "Se l'erinni muore al di fuori dei Nove Inferi, il suo corpo si dissolve in fumo sulfureo ed essa ne acquisisce uno nuovo all'istante, tornando in vita con tutti i suoi punti ferita da qualche parte nei Nove Inferi."
actions: 
- name: "Multiattacco"
  desc: "L'erinni effettua tre attacchi Spada inaridente e può usare Corda avvinghiante."
- name: "Spada inaridente"
  desc: "*Tiro per colpire in mischia:* +8, portata 1,5 m. *Colpito:* 13 (2d8 + 4) danni taglienti più 11 (2d10) danni necrotici."
- name: "Corda avvinghiante (richiede Corda magica)"
  desc: "*Tiro salvezza su Forza:* CD 16, una creatura che l'erinni è in grado di vedere entro 36 metri. *Fallimento:* 14 (4d6) danni da forza, e il bersaglio è trattenuto finché la corda non viene distrutta, finché l'erinni non usa un'azione bonus per liberare il bersaglio, o finché l'erinni non usa nuovamente Corda avvinghiante."
bonus_actions: []
reactions: 
- name: "Parata"
  desc: "*Attivazione:* l'erinni viene colpita da un tiro per colpire in mischia mentre tiene in mano un'arma.\n\n*Esito:* l'erinni aggiunge 4 alla sua CA contro quell'attacco, aumentando le probabilità che il colpo non vada a segno."
legendary_actions: []
lair_actions: []
tipo_creatura: "Immondo (diavolo)"
dimensione: "Medio"
allineamento: "legale malvagio"
classe_armatura: 18
iniziativa: 7
hp: 178
hit_dice: "21d8 + 84"
speed: "camminata: 9 m, volo: 18 m"
xp: 8400
bonus_competenza: 4
statblock: true
---
# Erinni

````tabs
tab: Scheda
```statblock
monster: Erinni
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Medio Immondo (diavolo), legale malvagio
> CA: 18
> PF: 178 (21d8 + 84)
> Velocita: camminata: 9 m, volo: 18 m
> GS: 12 (PE 8.400; BC +4)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 18 | 4 | 4 |
| Destrezza | 16 | 3 | 7 |
| Costituzione | 18 | 4 | 8 |
| Intelligenza | 14 | 2 | 2 |
| Saggezza | 14 | 2 | 2 |
| Carisma | 18 | 4 | 8 |
## Abilita

percezione: 6, persuasione: 8
## Sensi

percezione_passiva: 16, vista_pura: 36 m
## Lingue

Infernale, telepatia 36 m
tab: Azioni
## Tratti

### Corda magica

L'erinni possiede una corda magica. Finché la porta con sé, può usare l'azione Corda avvinghiante. La corda ha CA 20, 90 PF, e immunità ai danni psichici e da veleno. La corda si trasforma in polvere se viene ridotta a 0 punti ferita, se si trova a 1,5 metri o più dall'erinni per 1 ora o più, o se l'erinni muore. Se la corda è danneggiata o distrutta, l'erinni può ripristinarla interamente al termine di un riposo breve o lungo.

### Resistenza alla magia

L'erinni dispone di vantaggio ai tiri salvezza contro incantesimi e altri effetti magici.

### Ristoro diabolico

Se l'erinni muore al di fuori dei Nove Inferi, il suo corpo si dissolve in fumo sulfureo ed essa ne acquisisce uno nuovo all'istante, tornando in vita con tutti i suoi punti ferita da qualche parte nei Nove Inferi.
## Azioni

### Multiattacco

L'erinni effettua tre attacchi Spada inaridente e può usare Corda avvinghiante.

### Spada inaridente

*Tiro per colpire in mischia:* +8, portata 1,5 m. *Colpito:* 13 (2d8 + 4) danni taglienti più 11 (2d10) danni necrotici.

### Corda avvinghiante (richiede Corda magica)

*Tiro salvezza su Forza:* CD 16, una creatura che l'erinni è in grado di vedere entro 36 metri. *Fallimento:* 14 (4d6) danni da forza, e il bersaglio è trattenuto finché la corda non viene distrutta, finché l'erinni non usa un'azione bonus per liberare il bersaglio, o finché l'erinni non usa nuovamente Corda avvinghiante.
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.