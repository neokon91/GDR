---
id: "srd-arto_di_troll"
srd_id: "arto_di_troll"
nome: "Arto di troll"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Arto di troll"
type: "Gigante"
size: "piccolo"
alignment: "caotico malvagio"
ac: 13
stats: [18, 12, 10, 1, 9, 1]
saves: {}
skillsaves: {}
damage_vulnerabilities: 
damage_resistances: 
damage_immunities: 
condition_immunities: 
senses: "percezione_passiva: 9, scurovisione: 18 m"
languages: "nessuna"
gear: []
traits: 
- name: "Generare troll"
  desc: "Stranamente, l'arto possiede gli stessi sensi di un troll integro. Se non viene distrutto entro 24 ore, tira 1d12. Con un risultato di 12, l'arto diventa un troll, altrimenti avvizzisce e si decompone."
- name: "Rigenerazione"
  desc: "L'arto recupera 5 punti ferita all'inizio di ogni suo turno. Se l'arto subisce danni da acido o fuoco, questo tratto non funziona all'inizio del suo turno successivo. L'arto muore solo se inizia il suo turno con 0 punti ferita e non si rigenera."
actions: 
- name: "Squarcio"
  desc: "*Tiro per colpire in mischia:* +6, portata 1,5 m. *Colpito:* 9 (2d4 + 4) danni taglienti."
bonus_actions: []
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Gigante"
dimensione: "Piccolo"
allineamento: "caotico malvagio"
classe_armatura: 13
iniziativa: 1
hp: 14
hit_dice: "4d6"
speed: "camminata: 6 m"
cr: 0.5
xp: 100
bonus_competenza: 2
statblock: true
---
# Arto di troll

```statblock
monster: Arto di troll
```

> [!infobox|wiki]- Mostro SRD
> Tipo: Piccolo Gigante, caotico malvagio
> CA: 13
> PF: 14 (4d6)
> Velocita: camminata: 6 m
> GS: 1/2 (PE 100; BC +2)

## Caratteristiche

| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 18 | 4 | 4 |
| Destrezza | 12 | 1 | 1 |
| Costituzione | 10 | 0 | 0 |
| Intelligenza | 1 | -5 | -5 |
| Saggezza | 9 | -1 | -1 |
| Carisma | 1 | -5 | -5 |

## Sensi

percezione_passiva: 9, scurovisione: 18 m

## Lingue

nessuna

## Tratti

### Generare troll

Stranamente, l'arto possiede gli stessi sensi di un troll integro. Se non viene distrutto entro 24 ore, tira 1d12. Con un risultato di 12, l'arto diventa un troll, altrimenti avvizzisce e si decompone.

### Rigenerazione

L'arto recupera 5 punti ferita all'inizio di ogni suo turno. Se l'arto subisce danni da acido o fuoco, questo tratto non funziona all'inizio del suo turno successivo. L'arto muore solo se inizia il suo turno con 0 punti ferita e non si rigenera.

## Azioni

### Squarcio

*Tiro per colpire in mischia:* +6, portata 1,5 m. *Colpito:* 9 (2d4 + 4) danni taglienti.

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.