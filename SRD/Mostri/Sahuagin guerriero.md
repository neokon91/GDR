---
id: "srd-sahuagin_guerriero"
srd_id: "sahuagin_guerriero"
nome: "Sahuagin guerriero"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Sahuagin guerriero"
type: "Immondo"
size: "medio"
alignment: "legale malvagio"
ac: 12
stats: [13, 11, 12, 12, 13, 9]
saves: {}
skillsaves: 
  percezione: 5
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 15, scurovisione: 36 m"
languages: "Sahuagin"
cr: 0.5
traits: 
- name: "Anfibio limitato"
  desc: "Il sahuagin può respirare in aria e in acqua, ma deve immergersi in acqua almeno una volta ogni 4 ore per evitare di soffocare."
- name: "Frenesia di sangue"
  desc: "Il sahuagin dispone di vantaggio ai tiri per colpire contro ogni creatura che non possieda tutti i propri punti ferita."
- name: "Telepatia con gli squali"
  desc: "Il sahuagin può comandare magicamente ogni squalo entro 36 metri da sé, usando una forma di telepatia speciale."
actions: 
- name: "Multiattacco"
  desc: "Il sahuagin effettua due attacchi Artiglio."
- name: "Artiglio"
  desc: "*Tiro per colpire in mischia:* +3, portata 1,5 m. *Colpito:* 4 (1d6 + 1) danni taglienti."
bonus_actions: 
- name: "Carica acquatica"
  desc: "Il sahuagin nuota fino al massimo della sua velocità di nuoto in linea retta verso un nemico che è in grado di vedere."
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Immondo"
dimensione: "Medio"
allineamento: "legale malvagio"
classe_armatura: 12
iniziativa: 0
hp: 22
hit_dice: "4d8 + 4"
speed: "camminata: 9 m, nuoto: 12 m"
xp: 100
bonus_competenza: 2
statblock: true
---
# Sahuagin guerriero

````tabs
tab: Scheda
```statblock
monster: Sahuagin guerriero
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Medio Immondo, legale malvagio
> CA: 12
> PF: 22 (4d8 + 4)
> Velocita: camminata: 9 m, nuoto: 12 m
> GS: 1/2 (PE 100; BC +2)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 13 | 1 | 1 |
| Destrezza | 11 | 0 | 0 |
| Costituzione | 12 | 1 | 1 |
| Intelligenza | 12 | 1 | 1 |
| Saggezza | 13 | 1 | 1 |
| Carisma | 9 | -1 | -1 |
## Abilita

percezione: 5
## Sensi

percezione_passiva: 15, scurovisione: 36 m
## Lingue

Sahuagin
tab: Azioni
## Tratti

### Anfibio limitato

Il sahuagin può respirare in aria e in acqua, ma deve immergersi in acqua almeno una volta ogni 4 ore per evitare di soffocare.

### Frenesia di sangue

Il sahuagin dispone di vantaggio ai tiri per colpire contro ogni creatura che non possieda tutti i propri punti ferita.

### Telepatia con gli squali

Il sahuagin può comandare magicamente ogni squalo entro 36 metri da sé, usando una forma di telepatia speciale.
## Azioni

### Multiattacco

Il sahuagin effettua due attacchi Artiglio.

### Artiglio

*Tiro per colpire in mischia:* +3, portata 1,5 m. *Colpito:* 4 (1d6 + 1) danni taglienti.
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.