---
id: "srd-lamia"
srd_id: "lamia"
nome: "Lamia"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Lamia"
type: "Immondo"
size: "grande"
alignment: "caotico malvagio"
ac: 13
stats: [16, 13, 15, 14, 15, 16]
saves: {}
skillsaves: 
  inganno: 7
  intuizione: 4
  furtivita: 5
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 12, scurovisione: 18 m"
languages: "Abissale, Comune"
cr: 4
traits: []
actions: 
- name: "Multiattacco"
  desc: "Il lamia effettua due attacchi Artiglio. Può sostituire un attacco con un utilizzo di Tocco corruttore."
- name: "Artiglio"
  desc: "*Tiro per colpire in mischia:* +5, portata 1,5 m *Colpito:* 7 (1d8 + 3) danni taglienti più 7 (2d6) danni psichici."
- name: "Incantesimi"
  desc: "Il lamia lancia uno dei seguenti incantesimi, senza bisogno di componenti materiali, utilizzando Carisma come caratteristica da incantatore (CD del tiro salvezza sull'incantesimo 13): 1/giorno ciascuno: Costrizione, immagine maggiore, scrutare A volontà: Camuffare se stesso (può apparire come un bipede di taglia Grande o Media), illusione minore"
- name: "Tocco corruttore"
  desc: "*Tiro salvezza su Saggezza:* CD 13, una creatura che il lamia è in grado di vedere entro 1,5 metri. *Fallimento:* 13 (3d8) danni psichici, e il bersaglio è maledetto per 1 ora. Finché la maledizione perdura, il bersaglio è affascinato e avvelenato."
bonus_actions: 
- name: "Balzo"
  desc: "Il lamia salta fino a 9 metri usando 3 metri di movimento."
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Immondo"
dimensione: "Grande"
allineamento: "caotico malvagio"
classe_armatura: 13
iniziativa: 1
hp: 97
hit_dice: "13d10 + 26"
speed: "camminata: 12 m"
xp: 1100
bonus_competenza: 2
statblock: true
---
# Lamia

````tabs
tab: Scheda
```statblock
monster: Lamia
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Grande Immondo, caotico malvagio
> CA: 13
> PF: 97 (13d10 + 26)
> Velocita: camminata: 12 m
> GS: 4 (PE 1.100; BC +2)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 16 | 3 | 3 |
| Destrezza | 13 | 1 | 1 |
| Costituzione | 15 | 2 | 2 |
| Intelligenza | 14 | 2 | 2 |
| Saggezza | 15 | 2 | 2 |
| Carisma | 16 | 3 | 3 |
## Abilita

inganno: 7, intuizione: 4, furtivita: 5
## Sensi

percezione_passiva: 12, scurovisione: 18 m
## Lingue

Abissale, Comune
tab: Azioni
## Azioni

### Multiattacco

Il lamia effettua due attacchi Artiglio. Può sostituire un attacco con un utilizzo di Tocco corruttore.

### Artiglio

*Tiro per colpire in mischia:* +5, portata 1,5 m *Colpito:* 7 (1d8 + 3) danni taglienti più 7 (2d6) danni psichici.

### Incantesimi

Il lamia lancia uno dei seguenti incantesimi, senza bisogno di componenti materiali, utilizzando Carisma come caratteristica da incantatore (CD del tiro salvezza sull'incantesimo 13): 1/giorno ciascuno: Costrizione, immagine maggiore, scrutare A volontà: Camuffare se stesso (può apparire come un bipede di taglia Grande o Media), illusione minore

### Tocco corruttore

*Tiro salvezza su Saggezza:* CD 13, una creatura che il lamia è in grado di vedere entro 1,5 metri. *Fallimento:* 13 (3d8) danni psichici, e il bersaglio è maledetto per 1 ora. Finché la maledizione perdura, il bersaglio è affascinato e avvelenato.
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.