---
id: "srd-ettercap"
srd_id: "ettercap"
nome: "Ettercap"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Ettercap"
type: "Mostruosità"
size: "media"
alignment: "neutrale malvagio"
ac: 13
stats: [14, 15, 13, 7, 12, 8]
saves: {}
skillsaves: 
  furtivita: 4
  percezione: 3
  sopravvivenza: 3
damage_vulnerabilities: 
damage_resistances: 
damage_immunities: 
condition_immunities: 
senses: "percezione_passiva: 13, scurovisione: 18 m"
languages: "nessuna"
gear: []
traits: 
- name: "Andatura sulla ragnatela"
  desc: "L'ettercap ignora le restrizioni di movimento causate dalle ragnatele e conosce l'ubicazione esatta di ogni altra creatura a contatto con la stessa ragnatela."
- name: "Movimenti del ragno"
  desc: "L'ettercap può scalare le superfici difficili, compresi i soffitti, senza effettuare una prova di caratteristica."
actions: 
- name: "Multiattacco"
  desc: "L'ettercap effettua un attacco Morso e un attacco Artiglio."
- name: "Artiglio"
  desc: "*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 7 (2d4 + 2) danni taglienti."
- name: "Morso"
  desc: "*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 5 (1d6 + 2) danni perforanti più 2 (1d4) danni da veleno, e il bersaglio è avvelenato fino all'inizio del turno successivo dell'ettercap."
- name: "Filo di ragnatela (ricarica 5-6)"
  desc: "*Tiro salvezza su Destrezza:* CD 12, una creatura di taglia Grande o inferiore che l'ettercap è in grado di vedere entro 9 metri. *Fallimento:* il bersaglio è trattenuto finché la ragnatela non viene distrutta (CA 10; PF 5; vulnerabilità ai danni da fuoco; immunità ai danni contundenti, psichici e da veleno)."
bonus_actions: 
- name: "Riavvolgere"
  desc: "L'ettercap attira in linea retta verso di sé una creatura entro 9 metri trattenuta dal suo Filo di ragnatela per un massimo di 7,5 metri."
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Mostruosità"
dimensione: "Media"
allineamento: "neutrale malvagio"
classe_armatura: 13
iniziativa: 2
hp: 44
hit_dice: "8d8 + 8"
speed: "camminata: 9 m, scalata: 9 m"
cr: 2
xp: 450
bonus_competenza: 2
statblock: true
---
# Ettercap

```statblock
monster: Ettercap
```

> [!infobox|wiki]- Mostro SRD
> Tipo: Media Mostruosità, neutrale malvagio
> CA: 13
> PF: 44 (8d8 + 8)
> Velocita: camminata: 9 m, scalata: 9 m
> GS: 2 (PE 450; BC +2)

## Caratteristiche

| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 14 | 2 | 2 |
| Destrezza | 15 | 2 | 2 |
| Costituzione | 13 | 1 | 1 |
| Intelligenza | 7 | -2 | -2 |
| Saggezza | 12 | 1 | 1 |
| Carisma | 8 | -1 | -1 |

## Abilita

furtivita: 4, percezione: 3, sopravvivenza: 3

## Sensi

percezione_passiva: 13, scurovisione: 18 m

## Lingue

nessuna

## Tratti

### Andatura sulla ragnatela

L'ettercap ignora le restrizioni di movimento causate dalle ragnatele e conosce l'ubicazione esatta di ogni altra creatura a contatto con la stessa ragnatela.

### Movimenti del ragno

L'ettercap può scalare le superfici difficili, compresi i soffitti, senza effettuare una prova di caratteristica.

## Azioni

### Multiattacco

L'ettercap effettua un attacco Morso e un attacco Artiglio.

### Artiglio

*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 7 (2d4 + 2) danni taglienti.

### Morso

*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 5 (1d6 + 2) danni perforanti più 2 (1d4) danni da veleno, e il bersaglio è avvelenato fino all'inizio del turno successivo dell'ettercap.

### Filo di ragnatela (ricarica 5-6)

*Tiro salvezza su Destrezza:* CD 12, una creatura di taglia Grande o inferiore che l'ettercap è in grado di vedere entro 9 metri. *Fallimento:* il bersaglio è trattenuto finché la ragnatela non viene distrutta (CA 10; PF 5; vulnerabilità ai danni da fuoco; immunità ai danni contundenti, psichici e da veleno).

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.