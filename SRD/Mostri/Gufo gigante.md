---
id: "srd-gufo_gigante"
srd_id: "gufo_gigante"
nome: "Gufo gigante"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Gufo gigante"
type: "Celestiale"
size: "grande"
alignment: "neutrale"
ac: 12
stats: [13, 15, 12, 10, 14, 10]
saves: 
  wis: 4
skillsaves: 
  furtivita: 6
  percezione: 6
damage_vulnerabilities: 
damage_resistances: 
damage_immunities: 
condition_immunities: 
senses: "percezione_passiva: 16, scurovisione: 36 m"
languages: "Celestiale, capisce il Comune, l'Elfico e il Silvano ma non li parla"
gear: []
traits: 
- name: "Volo sfuggente"
  desc: "Il gufo non provoca attacchi di opportunità quando vola oltre la portata di un nemico."
actions: 
- name: "Speroni"
  desc: "*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 7 (1d10 + 2) danni taglienti."
- name: "Incantesimi"
  desc: "Il gufo lancia uno dei seguenti incantesimi, senza bisogno di componenti, utilizzando Saggezza come caratteristica da incantatore: 1/giorno: Chiaroveggenza A volontà: Individuazione del bene e del male, individuazione del magico"
bonus_actions: []
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Celestiale"
dimensione: "Grande"
allineamento: "neutrale"
classe_armatura: 12
iniziativa: 2
hp: 19
hit_dice: "3d10 + 3"
speed: "camminata: 5 m, volo: 18 m"
cr: 0.25
xp: 50
bonus_competenza: 2
statblock: true
---
# Gufo gigante

```statblock
monster: Gufo gigante
```

> [!infobox|wiki]- Mostro SRD
> Tipo: Grande Celestiale, neutrale
> CA: 12
> PF: 19 (3d10 + 3)
> Velocita: camminata: 5 m, volo: 18 m
> GS: 1/4 (PE 50; BC +2)

## Caratteristiche

| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 13 | 1 | 1 |
| Destrezza | 15 | 2 | 2 |
| Costituzione | 12 | 1 | 1 |
| Intelligenza | 10 | 0 | 0 |
| Saggezza | 14 | 2 | 4 |
| Carisma | 10 | 0 | 0 |

## Abilita

furtivita: 6, percezione: 6

## Sensi

percezione_passiva: 16, scurovisione: 36 m

## Lingue

Celestiale, capisce il Comune, l'Elfico e il Silvano ma non li parla

## Tratti

### Volo sfuggente

Il gufo non provoca attacchi di opportunità quando vola oltre la portata di un nemico.

## Azioni

### Speroni

*Tiro per colpire in mischia:* +4, portata 1,5 m. *Colpito:* 7 (1d10 + 2) danni taglienti.

### Incantesimi

Il gufo lancia uno dei seguenti incantesimi, senza bisogno di componenti, utilizzando Saggezza come caratteristica da incantatore: 1/giorno: Chiaroveggenza A volontà: Individuazione del bene e del male, individuazione del magico

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.