---
id: "srd-mephit_del_ghiaccio"
srd_id: "mephit_del_ghiaccio"
nome: "Mephit del ghiaccio"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Mephit del ghiaccio"
type: "Elementale"
size: "piccolo"
alignment: "neutrale malvagio"
ac: 11
stats: [7, 13, 10, 9, 11, 12]
saves: {}
skillsaves: 
  furtivita: 3
  percezione: 2
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 12, scurovisione: 18 m"
languages: "Primordiale (Aquan, Auran)"
cr: 0.5
traits: 
- name: "Esplosione mortale"
  desc: "Il mephit esplode quando muore. *Tiro salvezza su Costituzione:* CD 10, tutte le creature in un'emanazione di 1,5 metri di cui il mephit è il punto di origine. *Fallimento:* 5 (2d4) danni da freddo. *Successo:* danni dimezzati."
actions: 
- name: "Artiglio"
  desc: "*Tiro per colpire in mischia:* +3, portata 1,5 m. *Colpito:* 3 (1d4 + 1) danni taglienti più 2 (1d4) danni da freddo."
- name: "Nube di nebbia (1/giorno)"
  desc: "Il mephit lancia nube di nebbia senza bisogno di componenti, utilizzando Carisma come caratteristica da incantatore."
- name: "Soffio di gelo (ricarica 6)"
  desc: "*Tiro salvezza su Costituzione:* CD 10, tutte le creature in un cono di 4,5 metri. *Fallimento:* 7 (3d4) danni da freddo. *Successo:* danni dimezzati."
bonus_actions: []
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Elementale"
dimensione: "Piccolo"
allineamento: "neutrale malvagio"
classe_armatura: 11
iniziativa: 1
hp: 21
hit_dice: "6d6"
speed: "camminata: 9 m, volo: 9 m"
xp: 100
bonus_competenza: 2
statblock: true
---
# Mephit del ghiaccio

````tabs
tab: Scheda
```statblock
monster: Mephit del ghiaccio
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Piccolo Elementale, neutrale malvagio
> CA: 11
> PF: 21 (6d6)
> Velocita: camminata: 9 m, volo: 9 m
> GS: 1/2 (PE 100; BC +2)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 7 | -2 | -2 |
| Destrezza | 13 | 1 | 1 |
| Costituzione | 10 | 0 | 0 |
| Intelligenza | 9 | -1 | -1 |
| Saggezza | 11 | 0 | 0 |
| Carisma | 12 | 1 | 1 |
## Abilita

furtivita: 3, percezione: 2
## Sensi

percezione_passiva: 12, scurovisione: 18 m
## Lingue

Primordiale (Aquan, Auran)
tab: Azioni
## Tratti

### Esplosione mortale

Il mephit esplode quando muore. *Tiro salvezza su Costituzione:* CD 10, tutte le creature in un'emanazione di 1,5 metri di cui il mephit è il punto di origine. *Fallimento:* 5 (2d4) danni da freddo. *Successo:* danni dimezzati.
## Azioni

### Artiglio

*Tiro per colpire in mischia:* +3, portata 1,5 m. *Colpito:* 3 (1d4 + 1) danni taglienti più 2 (1d4) danni da freddo.

### Nube di nebbia (1/giorno)

Il mephit lancia nube di nebbia senza bisogno di componenti, utilizzando Carisma come caratteristica da incantatore.

### Soffio di gelo (ricarica 6)

*Tiro salvezza su Costituzione:* CD 10, tutte le creature in un cono di 4,5 metri. *Fallimento:* 7 (3d4) danni da freddo. *Successo:* danni dimezzati.
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.