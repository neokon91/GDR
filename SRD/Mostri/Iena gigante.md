---
id: "srd-iena_gigante"
srd_id: "iena_gigante"
nome: "Iena gigante"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
fonti: ["[[SRD/Licenza SRD]]", "[[SRD/Mostri/Iena gigante]]"]
riferimenti_srd: ["[[SRD/Mostri/Iena gigante]]"]
riferimenti_regola: []
sezioni_collegate: []
blocchi_collegati: ["[[SRD/Mostri/Iena gigante#^srd-iena_gigante-morso]]", "[[SRD/Mostri/Iena gigante#^srd-iena_gigante-furia-1-giorno]]"]
tabelle_collegate: []
tags: ["dnd55/srd", "dnd55/creatura"]
name: "Iena gigante"
type: "Bestia"
size: "grande"
alignment: "senza allineamento"
ac: 12
stats: [16, 14, 14, 2, 12, 7]
saves: {}
skillsaves: 
  percezione: 3
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 13, scurovisione: 18 m"
languages: "nessuna"
cr: 1
traits: []
actions: 
- name: "Morso"
  desc: "*Tiro per colpire in mischia:* +5, portata 1,5 m. *Colpito:* 10 (2d6 + 3) danni perforanti."
bonus_actions: 
- name: "Furia (1/giorno)"
  desc: "Subito dopo aver inflitto danni a una creatura già sanguinante, la iena si muove fino a metà della sua velocità, ed effettua un attacco Morso."
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Bestia"
dimensione: "Grande"
allineamento: "senza allineamento"
classe_armatura: 12
iniziativa: 2
hp: 45
hit_dice: "6d10 + 12"
speed: "camminata: 15 m"
xp: 200
bonus_competenza: 2
statblock: true
---
# Iena gigante

````tabs
tab: Scheda
```statblock
monster: Iena gigante
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Grande Bestia, senza allineamento
> CA: 12
> PF: 45 (6d10 + 12)
> Velocita: camminata: 15 m
> GS: 1 (PE 200; BC +2)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 16 | 3 | 3 |
| Destrezza | 14 | 2 | 2 |
| Costituzione | 14 | 2 | 2 |
| Intelligenza | 2 | -4 | -4 |
| Saggezza | 12 | 1 | 1 |
| Carisma | 7 | -2 | -2 |
## Abilita

percezione: 3
## Sensi

percezione_passiva: 13, scurovisione: 18 m
## Lingue

nessuna
tab: Azioni
## Azioni

### Morso

*Tiro per colpire in mischia:* +5, portata 1,5 m. *Colpito:* 10 (2d6 + 3) danni perforanti.
^srd-iena_gigante-morso
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.