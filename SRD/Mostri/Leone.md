---
id: "srd-leone"
srd_id: "leone"
nome: "Leone"
categoria: "srd"
tipo: "mostro"
stato: "pronto"
canonico: false
fonte: "SRD 5.2.1"
licenza: "CC-BY-4.0"
repository: "neokon91/DND-SRD-IT"
generato_da: "import_srd"
name: "Leone"
type: "Bestia"
size: "grande"
alignment: "senza allineamento"
ac: 12
stats: [17, 15, 11, 3, 12, 8]
saves: {}
skillsaves: 
  furtivita: 4
  percezione: 3
damage_vulnerabilities:
damage_resistances:
damage_immunities:
condition_immunities:
senses: "percezione_passiva: 13, scurovisione: 18 m"
languages: "nessuna"
cr: 1
traits: 
- name: "Balzo in corsa"
  desc: "Dopo una rincorsa di 3 metri, il leone può effettuare un salto in lungo di un massimo di 7,5 metri."
- name: "Tattiche del branco"
  desc: "Il leone dispone di vantaggio a un tiro per colpire contro una creatura se almeno uno degli alleati dell'animale si trova entro 1,5 metri dalla creatura e non ha la condizione incapacitato."
actions: 
- name: "Multiattacco"
  desc: "Il leone effettua due attacchi Squarcio. Può sostituire un attacco con un utilizzo di Ruggito."
- name: "Squarcio"
  desc: "*Tiro per colpire in mischia:* +5, portata 1,5 m. *Colpito:* 7 (1d8 + 3) danni taglienti."
- name: "Ruggito"
  desc: "*Tiro salvezza su Saggezza:* CD 11, una creatura entro 4,5 metri. *Fallimento:* il bersaglio è spaventato fino all'inizio del turno successivo del leone."
bonus_actions: []
reactions: []
legendary_actions: []
lair_actions: []
tipo_creatura: "Bestia"
dimensione: "Grande"
allineamento: "senza allineamento"
classe_armatura: 12
iniziativa: 2
hp: 22
hit_dice: "4d10"
speed: "camminata: 15 m"
xp: 200
bonus_competenza: 2
statblock: true
---
# Leone

````tabs
tab: Scheda
```statblock
monster: Leone
```
tab: Dettagli
> [!infobox|wiki]- Mostro SRD
> Tipo: Grande Bestia, senza allineamento
> CA: 12
> PF: 22 (4d10)
> Velocita: camminata: 15 m
> GS: 1 (PE 200; BC +2)
## Caratteristiche
| Caratteristica | Punteggio | Modificatore | Tiro Salvezza |
| --- | --- | --- | --- |
| Forza | 17 | 3 | 3 |
| Destrezza | 15 | 2 | 2 |
| Costituzione | 11 | 0 | 0 |
| Intelligenza | 3 | -4 | -4 |
| Saggezza | 12 | 1 | 1 |
| Carisma | 8 | -1 | -1 |
## Abilita

furtivita: 4, percezione: 3
## Sensi

percezione_passiva: 13, scurovisione: 18 m
## Lingue

nessuna
tab: Azioni
## Tratti

### Balzo in corsa

Dopo una rincorsa di 3 metri, il leone può effettuare un salto in lungo di un massimo di 7,5 metri.

### Tattiche del branco

Il leone dispone di vantaggio a un tiro per colpire contro una creatura se almeno uno degli alleati dell'animale si trova entro 1,5 metri dalla creatura e non ha la condizione incapacitato.
## Azioni

### Multiattacco

Il leone effettua due attacchi Squarcio. Può sostituire un attacco con un utilizzo di Ruggito.

### Squarcio

*Tiro per colpire in mischia:* +5, portata 1,5 m. *Colpito:* 7 (1d8 + 3) danni taglienti.

### Ruggito

*Tiro salvezza su Saggezza:* CD 11, una creatura entro 4,5 metri. *Fallimento:* il bersaglio è spaventato fino all'inizio del turno successivo del leone.
````

> [!info] Licenza
> Questa nota include materiale tratto dal System Reference Document 5.2.1 di Wizards of the Coast LLC, disponibile su https://www.dndbeyond.com/srd e concesso in licenza CC-BY-4.0.