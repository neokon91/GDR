...
const SOURCE = "Dev/TemplateFactory/modules/quality_report_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_quality_report.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+qualityReportViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Quality Report non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["operational_gaps", "public_risks", "public_missing_text", "screenshot_ready"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Quality Report mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "quality_report_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_quality_report_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Quality Report cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Quality Report non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Quality Report cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Quality Report cockpit renderizzato: ${OUT}`);
}
