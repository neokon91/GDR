...
const SOURCE = "Dev/TemplateFactory/modules/living_world_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_living_world.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+livingWorldViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Motore Mondo Vivo non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["continuity", "targets", "closable", "gaps", "powers", "economy", "history", "public_canon"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda mondo vivo mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "living_world_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_living_world_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Motore Mondo Vivo cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Motore Mondo Vivo non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Motore Mondo Vivo cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Motore Mondo Vivo cockpit renderizzato: ${OUT}`);
}
