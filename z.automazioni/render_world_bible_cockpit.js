...
const SOURCE = "Dev/TemplateFactory/modules/world_bible_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_world_bible.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+worldBibleViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Bibbia del Mondo non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["world_identity", "article_gaps", "public_safety", "playable_gaps"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Bibbia del Mondo mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "world_bible_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_world_bible_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Bibbia del Mondo cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Bibbia del Mondo non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Bibbia del Mondo cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Bibbia del Mondo cockpit renderizzato: ${OUT}`);
}
