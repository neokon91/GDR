...
const SOURCE = "Dev/TemplateFactory/modules/compendium_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_compendium.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+compendiumViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Compendium non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["archive", "without_use", "open_gaps", "history_links", "pressure", "map_links"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Compendium mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "compendium_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_compendium_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Compendium cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Compendium non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Compendium cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Compendium cockpit renderizzato: ${OUT}`);
}
