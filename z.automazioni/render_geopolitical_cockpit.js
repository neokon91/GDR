...
const SOURCE = "Dev/TemplateFactory/modules/geopolitical_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_geopolitical.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+geopoliticalViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Geopolitical Dashboard non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["territories", "relations", "borders", "resources", "economic_nodes", "gaps"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda geopolitica mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "geopolitical_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_geopolitical_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Geopolitical Dashboard cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Geopolitical Dashboard non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Geopolitical Dashboard cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Geopolitical Dashboard cockpit renderizzato: ${OUT}`);
}
