...
const SOURCE = "Dev/TemplateFactory/modules/preparation_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_preparation.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+preparationViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Preparazione non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["candidate_sessions", "anchors", "missions", "pressures", "people", "encounters", "handouts", "maps"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Preparazione mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "preparation_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_preparation_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Preparazione Sessione cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Preparazione Sessione non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Preparazione Sessione cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Preparazione Sessione cockpit renderizzato: ${OUT}`);
}
