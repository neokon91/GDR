...
const SOURCE = "Dev/TemplateFactory/modules/dm_dashboard_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_dm_dashboard.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+dmDashboardViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export DM Dashboard non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["sessions", "pressures", "materials", "inbox"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda DM Dashboard mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "dm_dashboard_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_dm_dashboard_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto DM Dashboard cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto DM Dashboard non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`DM Dashboard cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`DM Dashboard cockpit renderizzato: ${OUT}`);
}
