...
const SOURCE = "Dev/TemplateFactory/modules/atlas_cockpit.yaml";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+atlasViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export atlas non collegato (${view})`);
            }
        }
    }
...
function buildPayload() {
    ...
    if (contract.id !== "atlas_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_atlas_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Atlante cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Atlante cockpit non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Atlante cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Atlante cockpit renderizzato: ${OUT}`);
}
