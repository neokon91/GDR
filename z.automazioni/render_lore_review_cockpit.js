...
const SOURCE = "Dev/TemplateFactory/modules/lore_review_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_lore_review.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+loreReviewViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Revisione Lore non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["completion", "playability", "anchors", "mysteries", "history", "pressures"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Revisione Lore mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "lore_review_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_lore_review_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Revisione Lore cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Revisione Lore non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Revisione Lore cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Revisione Lore cockpit renderizzato: ${OUT}`);
}
