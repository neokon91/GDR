...
const SOURCE = "Dev/TemplateFactory/modules/live_table_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_live_table.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+liveTableViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export tavolo live non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["live_notes", "pressures", "people", "materials", "post_bridge"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda tavolo live mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "live_table_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_live_table_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Durante il Gioco cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Durante il Gioco non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Durante il Gioco cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Durante il Gioco cockpit renderizzato: ${OUT}`);
}
