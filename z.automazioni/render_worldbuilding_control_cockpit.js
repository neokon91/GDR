...
const SOURCE = "Dev/TemplateFactory/modules/worldbuilding_control_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_worldbuilding_control.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+worldbuildingControlViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export controllo worldbuilding non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["depth", "connections", "canon", "playability", "player_safe", "ready_unused"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda audit mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "worldbuilding_control_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_worldbuilding_control_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Controllo Worldbuilding cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Controllo Worldbuilding non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Controllo Worldbuilding cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Controllo Worldbuilding cockpit renderizzato: ${OUT}`);
}
