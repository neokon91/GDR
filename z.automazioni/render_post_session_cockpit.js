...
const SOURCE = "Dev/TemplateFactory/modules/post_session_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_post_session.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+postSessionViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Post Sessione non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["live_notes", "canon_decisions", "recaps", "consequences", "impacted", "next_session"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Post Sessione mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "post_session_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_post_session_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Post Sessione cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Post Sessione non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Post Sessione cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Post Sessione cockpit renderizzato: ${OUT}`);
}
