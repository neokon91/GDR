...
const SOURCE = "Dev/TemplateFactory/modules/lore_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_lore.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+loreViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Lore Hub non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["signals", "canon_decisions", "mysteries", "history", "cultures", "powers", "maps", "materials"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda lore mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "lore_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_lore_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Lore Hub cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Lore Hub non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Lore Hub cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Lore Hub cockpit renderizzato: ${OUT}`);
}
