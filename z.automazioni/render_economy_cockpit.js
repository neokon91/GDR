...
const SOURCE = "Dev/TemplateFactory/modules/economy_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_economy.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+economyViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Economia E Rotte non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["routes", "resources", "markets", "controllers", "dependencies", "unpropagated", "gaps"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda economia mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "economy_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_economy_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Economia E Rotte cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Economia E Rotte non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Economia E Rotte cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Economia E Rotte cockpit renderizzato: ${OUT}`);
}
