...
const SOURCE = "Dev/TemplateFactory/modules/campaign_builder_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_campaign_builder.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+campaignBuilderViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Campagna da Ambientazione non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["opportunities", "fronts", "campaigns", "sessions", "gaps"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Campagna da Ambientazione mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "campaign_builder_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_campaign_builder_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Campagna da Ambientazione cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Campagna da Ambientazione non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Campagna da Ambientazione cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Campagna da Ambientazione cockpit renderizzato: ${OUT}`);
}
