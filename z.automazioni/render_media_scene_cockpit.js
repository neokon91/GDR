...
const SOURCE = "Dev/TemplateFactory/modules/media_scene_cockpit.yaml";
...
function validateDashboard(contract) {
    ...
    const mediaRuntime = readTextRel(ROOT, "z.engine/session_media_scene.js", "");
    ...
    if (!text) {
        ...
        if (text.split(/\r?\n/).length > 95) {
            fail(`${page}: superficie troppo lunga per un cockpit media compatto`);
        }
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+mediaSceneViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export media scene non collegato (${view})`);
            }
            if (!new RegExp(`function\\s+${escapeRegExp(view)}\\b`).test(mediaRuntime)) {
                fail(`z.engine/session_media_scene.js: funzione runtime mancante (${view})`);
            }
        }
    }
...
function validateQueues(contract) {
    return requiredList(contract.queues, "queues").map((queue, index) => ({
        ...
        label: requiredText(queue.label, `queues.${queue.id}.label`),
        table_columns: requiredStringArray(queue.table_columns, `queues.${queue.id}.table_columns`)
    }));
}

function validatePatterns(contract) {
    return requiredList(contract.patterns, "patterns").map((pattern, index) => ({
        id: requiredText(pattern.id, `patterns[${index}].id`),
        label: requiredText(pattern.label, `patterns.${pattern.id}.label`),
        example: requiredText(pattern.example, `patterns.${pattern.id}.example`),
        use_when: requiredText(pattern.use_when, `patterns.${pattern.id}.use_when`)
    }));
}
...
function buildPayload() {
    ...
    if (contract.id !== "media_scene_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    const patterns = validatePatterns(contract);
    const surfaces = validateSurfaces(contract);
    validateGeneratedRootsStayUntracked();

    return {
        generated_by: "render_media_scene_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Media Scene cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Media Scene cockpit non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Media Scene cockpit OK: ${payload.queues.length} code e ${payload.surfaces.length} superfici verificate.`);
        return;
    }
