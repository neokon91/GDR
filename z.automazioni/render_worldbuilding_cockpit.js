...
const SOURCE = "Dev/TemplateFactory/modules/worldbuilding_cockpit.yaml";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const section of sections) {
            if (!new RegExp(`^##\\s+${section.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*$`, "m").test(text)) {
    ...
    for (const view of runtimeViews) {
        if (!new RegExp(`\\b${view}\\b`).test(runtime)) {
            fail(`z.engine/session_views.js: runtime view mancante (${view})`);
        }
    }
...
function buildPayload() {
    ...
    if (contract.id !== "worldbuilding_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_worldbuilding_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Worldbuilder cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error(`Contratto Worldbuilder cockpit non aggiornato: eseguire npm run sync:sources`);
            process.exit(1);
        }
        console.log(`Worldbuilder cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Worldbuilder cockpit renderizzato: ${OUT}`);
}
