...
SOURCE = ROOT / "Dev" / "TemplateFactory" / "modules" / "root_pages.yaml"
...
def normalize_page(record: dict[str, Any], index: int, errors: list[str]) -> dict[str, Any] | None:
    ...
    elif path not in ALLOWED_ROOT_PAGES:
        fail(local_errors, f"{SOURCE.relative_to(ROOT)}: root page non ammessa ({path})")
    if not title:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera le pagine root finali da YAML/Jinja.")
    ...
    if args.list_targets:
        if errors:
            print("Pagine root non valide:", file=sys.stderr)
            for error in errors:
    ...
    print(f"Root pages {'OK' if args.check else 'generate'}: {len(rendered)} file da YAML/Jinja.")
    return 0
