...
const SOURCE = "Dev/TemplateFactory/modules/offscreen_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_offscreen.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+offscreenViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export fuori scena non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["actors", "clocks", "consequences", "secrets", "bridge"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda fuori scena mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "offscreen_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_offscreen_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Cosa Succede Fuori Scena cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Cosa Succede Fuori Scena non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Cosa Succede Fuori Scena cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Cosa Succede Fuori Scena cockpit renderizzato: ${OUT}`);
}
