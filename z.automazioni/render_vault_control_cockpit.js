...
const SOURCE = "Dev/TemplateFactory/modules/vault_control_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_vault_control.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+vaultControlViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Controllo Vault non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["attention", "maps", "table_ready", "campaign_open"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Controllo Vault mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "vault_control_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_vault_control_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Controllo Vault cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Controllo Vault non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Controllo Vault cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Controllo Vault cockpit renderizzato: ${OUT}`);
}
