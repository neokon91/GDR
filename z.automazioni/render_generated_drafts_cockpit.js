...
const SOURCE = "Dev/TemplateFactory/modules/generated_drafts_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_generated_drafts.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+generatedDraftsViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export Smistamento Bozze non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["draft_queue", "ready_to_link", "unanchored", "destinations", "resolved"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda Smistamento Bozze mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "generated_drafts_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_generated_drafts_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Smistamento Bozze cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Smistamento Bozze non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Smistamento Bozze cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Smistamento Bozze cockpit renderizzato: ${OUT}`);
}
