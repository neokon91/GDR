...
const SOURCE = "Dev/TemplateFactory/modules/canon_control_cockpit.yaml";
...
const RUNTIME_MODULE = "z.engine/session_canon_control.js";
...
function validateDashboard(contract) {
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+canonControlViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export controllo canone non collegato (${view})`);
            }
...
function validateQueues(contract) {
    const expected = new Set(["truth", "rumors", "contradictions", "provenance", "retcons", "decisions"]);
    ...
    for (const id of expected) {
        if (!queues.some(queue => queue.id === id)) fail(`${SOURCE}: coda canone mancante (${id})`);
    }
    return queues;
}
...
function buildPayload() {
    ...
    if (contract.id !== "canon_control_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    return {
        generated_by: "render_canon_control_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Controllo Canone cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Controllo Canone non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Controllo Canone cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Controllo Canone cockpit renderizzato: ${OUT}`);
}
