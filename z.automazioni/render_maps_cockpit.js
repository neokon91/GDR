...
const SOURCE = "Dev/TemplateFactory/modules/maps_cockpit.yaml";
...
function validateDashboard(contract) {
    ...
    const mapRuntime = readTextRel(ROOT, "z.engine/session_maps.js", "");
    ...
    if (!text) {
        ...
        for (const view of runtimeViews) {
            ...
            if (!new RegExp(`\\b${view}:\\s+mapViews\\.${view}\\b`).test(runtime)) {
                fail(`z.engine/session_views.js: export mappe non collegato (${view})`);
            }
            if (!new RegExp(`function\\s+${escapeRegExp(view)}\\b`).test(mapRuntime)) {
                fail(`z.engine/session_maps.js: funzione runtime mancante (${view})`);
            }
        }
    }
...
function validateCoordinatesContract() {
    const bases = loadYaml(BASES_SOURCE);
    const atlas = bases.views?.atlas_maps ?? {};
    if (!Array.isArray(atlas.required_fields) || !atlas.required_fields.includes("coordinates")) {
        fail(`${BASES_SOURCE}: atlas_maps deve richiedere coordinates`);
    }
    if (atlas.map_view !== true) {
        fail(`${BASES_SOURCE}: atlas_maps deve dichiarare map_view: true`);
    }
}
...
function buildPayload() {
    ...
    if (contract.id !== "maps_cockpit") fail(`${SOURCE}: id non valido`);
    ...
    validateCoordinatesContract();
    validateGeneratedRootsStayUntracked();

    return {
        generated_by: "render_maps_cockpit",
        source: SOURCE,
...
function main() {
    ...
    if (errors.length) {
        console.error("Contratto Mappe cockpit non valido:");
    ...
    if (CHECK_ONLY) {
        ...
        if (current !== rendered) {
            console.error("Contratto Mappe cockpit non aggiornato: eseguire npm run sync:sources");
            process.exit(1);
        }
        console.log(`Mappe cockpit OK: ${payload.surfaces.length} superfici e ${payload.queues.length} code verificate.`);
        return;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, rendered, "utf8");
    console.log(`Mappe cockpit renderizzato: ${OUT}`);
}
