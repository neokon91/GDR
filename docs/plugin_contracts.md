...
| **Fantasy Statblocks** | Union per id dei layout in `obsidian-5e-statblocks/data.json` (NON cambia il default) + `diceRolling: true`. Due layout IT: `statblock.layout` (5.5e fedele, default) e `statblock.layout_5e` (5e classico). `srd_statblock_yaml` mappa i mostri SRD su TUTTI i campi 2024 (initiative/saves/skillsaves/resist./immunità/gear/bonus_actions/reactions/leggendarie+desc/pb). Template creatura: 2 tab (5.5e inline + 5e via `monster:`, dati condivisi); le note creatura hanno `statblock: inline`. **Transizione**: lo **statblock nativo** del plugin (```gdr statblock`) affianca/sostituirà FS per la consultazione (ritiro pianificato, non fatto). | `Dev/Source/statblocks/*.json`, `srd_statblock_yaml`, `creature.md.j2` |
| **Initiative Tracker** | Blocco ` ```encounter ` (combattimento + iniziativa, legge il bestiario FS); `aggiorna_encounter` riscrive creature **+ alleati** (flag `ally`) dalle relazioni e applica gli **override HP/CA/init** dal campo `varianti` (boss/gregari, incontri ripetibili); il pannello *difficoltà* (`renderEncounter`) stima il budget XP 2024. I PG entrano via *Party* nelle impostazioni del plugin. **Transizione**: la **Board** nativa (motore `regole`) è il nuovo tracker; IT resta legacy finché il flusso ```encounter non è ricucito alla Board (ritiro pianificato). | `encounter.md.j2`, `meta_actions.js`, `views.js` |
| **Dice Roller** | Macro `tiri()` (d20 Normale/Vant./Svant.) + **scheda PG**: tiri col bonus reale che leggono il frontmatter (`dice: 1d20 + mod_<car>`, TS, abilità, iniziativa, TS-morte) — ✅ verificato in-app; **Attacchi con maestria** (`renderAttacchi`): per ogni arma di cui il PG ha padronanza, tiro per colpire + danni + effetto della maestria (catalogo armi SRD); liste di consultazione richiamabili (`dice: [[Nota]]`, incorpora l'elenco — il single-pick richiede il formato tabella DR); più `diceRolling` negli statblock. | `scheda_pg_rules()`, macro `tiri()`/`attacchi()`, `pg.md.j2` |
