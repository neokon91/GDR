...
FROM "Mondi/Timeline"
WHERE file.name != "Timeline" AND stato_canonico = "canonico"
...
FROM "Mondi/Timeline"
WHERE file.name != "Timeline" AND stato_canonico != "archiviata" AND (entita_impattate OR propaga_a OR stato_mondo OR tracciati)
SORT data_mondo ASC, file.name ASC
```
...
FROM "Mondi/Timeline"
WHERE file.name != "Timeline" AND stato_canonico != "canonico" AND stato_canonico != "archiviata"
SORT data_mondo ASC, file.name ASC
```
