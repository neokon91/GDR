...
function validateMinimumScenario(errors, contract) {
    // La fixture continuita e il contratto demo devono coprire la stessa catena narrativa minima.
    ...
    for (const category of FIXTURE_SCENARIO.requiredCategories) {
        if (!requiredCategories.has(category)) {
            fail(errors, `demo_contract.final_demo_minimum.categories non copre fixture continuita (${category})`);
        }
    }
