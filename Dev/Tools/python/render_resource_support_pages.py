...
SOURCE = ROOT / "Dev" / "Source" / "YAML" / "render" / "resource_support_pages.yaml"
ASSET_ROOT = ROOT / "Dev" / "Source" / "Assets"
...
def load_body(record: dict[str, Any], rel_path: str, errors: list[str]) -> str:
    inline_body = record.get("body")
    body_file = str(record.get("body_file") or "").replace("\\", "/").strip()

    if inline_body not in (None, "") and body_file:
        fail(errors, f"{rel_path}: usare body oppure body_file, non entrambi")
        return ""

    if body_file:
        asset = (ROOT / body_file).resolve()
        try:
            asset.relative_to(ASSET_ROOT.resolve())
        except ValueError:
            fail(errors, f"{rel_path}: body_file fuori da Dev/Source/Assets ({body_file})")
            return ""
        if not asset.exists():
            fail(errors, f"{rel_path}: body_file mancante ({body_file})")
            return ""
        return asset.read_text(encoding="utf-8").strip()

    body = str(inline_body or "").strip()
    if "```compressed-json" in body:
        fail(errors, f"{rel_path}: body Excalidraw compresso deve stare in body_file")
    return body
...
def normalize_page(record: dict[str, Any], index: int, errors: list[str]) -> dict[str, Any] | None:
    ...
    body = load_body(record, path or f"pages[{index}]", local_errors)
