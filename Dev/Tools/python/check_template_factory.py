...
def validate_blueprints(modules: dict[str, dict], errors: list[str]) -> None:
    ...
    for name, blueprint in blueprints.items():
        ...
        if not template_ref:
            ...
            template_path = (JINJA / Path(str(template_ref)).name).resolve()
...
def validate_critical_rendered_generators(modules: dict[str, dict], errors: list[str]) -> None:
    ...
    for profile_id, rel_path in sorted(CRITICAL_RENDERED_GENERATORS.items()):
        if profile_id not in frontmatter_profiles:
            fail(f"template factory: profilo frontmatter critico mancante ({profile_id})", errors)
        if profile_id not in runtime_profiles:
            fail(f"template factory: profilo runtime critico mancante ({profile_id})", errors)

        path = ROOT / rel_path
        if not path.exists():
            fail(f"template factory: generatore critico mancante ({rel_path})", errors)
            continue

        source = path.read_text(encoding="utf-8")
        if f'renderFrontmatter("{profile_id}"' not in source:
            fail(f"template factory: {rel_path} non usa renderFrontmatter(\"{profile_id}\")", errors)
        if "return `---" in source:
            fail(f"template factory: {rel_path} contiene ancora frontmatter inline", errors)
