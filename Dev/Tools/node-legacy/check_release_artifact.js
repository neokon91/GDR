...
function readJsonRel(root, relPath, errors, fallback = {}) {
    try {
        return JSON.parse(fs.readFileSync(path.join(root, relPath), "utf8"));
    } catch (error) {
        fail(errors, `${relPath}: JSON release non leggibile (${error.message})`);
        return fallback;
    }
}
...
function validateOutput(errors) {
    ...
    if (!bridge.includes("z.automazioni/data/runtime/runtime_exports.json")) {
        fail(errors, "session_views.js della release non legge runtime_exports.json");
    }
    ...
    for (const marker of asArray(releaseBoundary.bridge_runtime_modules)) {
        ...
        if (!existsRel(OUT, modulePath)) fail(errors, `modulo bridge mancante nella release: ${modulePath}`);
    }
