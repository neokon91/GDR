...
const ZIP = `${OUT}.zip`;
const CONTRACT = "Dev/Source/YAML/canonical/plugin_contracts.yaml";
const RELEASE_BOUNDARY = "Dev/Source/YAML/quality/release_boundary.yaml";
const RELEASE_QUALITY_CONTRACT = "Dev/Source/YAML/quality/release_quality_contract.yaml";
const METABIND_INPUTS = "Dev/Source/YAML/json/metabind_inputs.yaml";
const METABIND_BUTTONS = "Dev/Source/YAML/json/metabind_buttons.yaml";
const WORKFLOWS = "Dev/Source/YAML/json/workflows.yaml";
...
function validateMetaBindTargets(metaBind, root, firstRun, communityPlugins = []) {
    ...
    if (!firstRun) {
        for (const fileRel of markdownFiles(ROOT)) {
            if (fileRel.startsWith("Dev/Examples/")) continue;
...
function validateRuntimeButtonReferences(root = ROOT) {
    ...
    const files = [
        ...markdownFiles(root).filter(file => !file.startsWith("Dev/Examples/")),
...
function validateDataviewSyntax(root = ROOT, firstRunOnly = false) {
    const files = firstRunOnly ? FIRST_RUN_PAGES : markdownFiles(root).filter(file => !file.startsWith("z.modelli/") && !file.startsWith("Dev/Examples/"));
    for (const fileRel of files) {
...
function validateReleaseFirstRun() {
    ...
    execFileSync("node", ["Dev/Tools/node-legacy/release_clean.js", "--quiet", "--out", OUT], { cwd: ROOT, stdio: "inherit" });
