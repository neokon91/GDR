...
const TEMPLATE_FACTORY_RENDERER = repoPath(ROOT, "Dev/Tools/python/render_template_factory.py");
...
function writeGeneratedReleaseNotes() {
    ...
    for (const [relPath, text] of Object.entries(renderedMaterializedUserFiles(ROOT))) {
...
function findSourceYamlModule(moduleName) {
    const yamlRoot = repoPath(ROOT, "Dev/Source/YAML");
    const candidates = [];
...
function copyRuntimeTemplateModules() {
    ...
    for (const moduleName of RUNTIME_TEMPLATE_MODULES) {
        const source = findSourceYamlModule(moduleName);
        if (!source || !fs.existsSync(source)) {
            console.error(`Modulo runtime release mancante: ${moduleName}`);
            process.exit(1);
        }
...
function materializeGeneratedReleaseRoots() {
    if (GENERATED_RELEASE_ROOTS.has("z.bases") || GENERATED_RELEASE_ROOTS.has("z.fileclass")) {
        execFileSync("python3", [
            repoPath(ROOT, "Dev/Tools/python/render_metadata_surfaces.py"),
            "--target-root",
            OUT
        ], {
            cwd: ROOT,
    ...
    if (GENERATED_RELEASE_ROOTS.has("SRD")) {
        execFileSync("node", [repoPath(ROOT, "Dev/Tools/node-legacy/import_srd.js")], {
            cwd: OUT,
            encoding: "utf8",
            stdio: QUIET ? "pipe" : "inherit",
