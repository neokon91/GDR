...
- **Fili narrativi**: scrivi in qualunque nota un promemoria con `- [ ] …` e un tag
  `#gancio` (da seminare) o `#trama` (filo aperto): compare in Home → *Al tavolo*.
- **Dove trovo le cose**: **Home** (plancia), **Indici** (Atlante/Bestiario/Cast/…),
  **🔎 Esplora il mondo** (filtra tutto senza codice), **SRD** (regole). Tutto nei
  **segnalibri** a sinistra. **Clicca una cartella** (es. *Divinità*) per il suo indice automatico.
...
---
> [!info] 📖 Vuoi il riferimento completo?
> Setup dei plugin, mappe, statblock, calendario, sito dei giocatori, tassonomia delle
> categorie… è tutto nel **[[Manuale]]** (nei segnalibri). Per giocare non ti serve subito:
> aprilo quando ti serve un dettaglio.
