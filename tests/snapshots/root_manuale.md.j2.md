...
> testo grezzo; senza **Meta Bind** i campi `INPUT`/`VIEW` (incluso il componente *Carattere*)
> restano testo grezzo.
...
- Ogni nota lore può aggiungere **Al tavolo** (dal bottone **＋ Componenti**: uso al
  tavolo, gancio, pressione): è la superficie giocabile, il cuore del vault.
...
- Per un **incontro**: *Crea → Incontro*, collega le creature (e gli eventuali **Alleati**:
  PNG/evocazioni) nel tab *Collegamenti*, poi — con la nota aperta — lancia il comando
  **«GDR: Schiera l'incontro nella Board»**: la Board si apre pre-popolata (nemici + i tuoi
  **PG** + alleati). Da lì tiri l'iniziativa e giochi (motore nativo: PF, condizioni, tiri).
...
- **Crea** una nota dai bottoni in Home (o nell'indice di dominio): il wizard chiede
  solo **nome** (e **tipo**, se la categoria ne ha). La nota nasce **snella** —
  identità, descrizione, Collegamenti — e la rifinisci con calma coi campi `INPUT`/slider.
- **＋ Componenti** — sotto ogni nota, il bottone **＋ Aggiungi componente** aggiunge
  *quando serve* le superfici "da tavolo": **Al tavolo** (uso al tavolo, gancio, pressione),
  **Clock** (sui fronti), **Carattere** (radar degli assi), **Cronologia**, **Vista**.
  Niente muro di roba alla creazione: la nota cresce a richiesta.
- **Collega** due note quando vuoi (anche dopo): nel tab *Collegamenti* premi
  **`Collega`**, scegli il tipo di relazione e la nota — scrive il link tipizzato e
  quello inverso. Il componente *Vista* mostra anche **"Citato da"** (chi ti referenzia).
...
Genera un **sito statico, in sola lettura e senza spoiler** da dare ai giocatori, **tutto
dentro Obsidian — niente terminale**: apri **[[Occhi del giocatore]]** e premi **Genera sito**
(scegli il livello di rivelazione). Il sito esce nella cartella **`Sito-giocatori/`** del vault:
apri `index.html` nel browser (clic destro sulla cartella → *Mostra nel Finder*) o pubblica la
cartella. Restano **sempre fuori**: le note `visibilita: dm`, gli strumenti del DM (incontri,
tiri `dice:`) e i segreti non ancora svelati.

**Rivelazione progressiva** — ogni nota ha, nel callout **⚙️ Opzioni**, un selettore 👁 *Rivelazione*:
...
Genera il sito al **livello a cui è arrivata la campagna** (lo scegli nel bottone *Genera
sito*): una nota entra nel portale solo se il suo tier è ≤ il livello scelto. Man mano che i
giocatori scoprono il mondo, alzi il livello e rigeneri. Per non condividere **mai** una nota
(appunti, meta), usa `visibilita: dm`.
